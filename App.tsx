    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                 {isLoading ? (
                    <div className="p-12 text-center text-gray-400 animate-pulse">Загрузка экспериментов...</div>
                 ) : (
                    <table className="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {experiments.length === 0 ? (
                                <tr>
                                    <td colSpan={5} className="px-6 py-12 text-center text-gray-500">Эксперименты не найдены</td>
                                </tr>
                            ) : experiments.map(exp => (
                                <tr key={exp.id}>
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        <div className="text-sm font-medium text-gray-900">{exp.name}</div>
                                        <div className="text-sm text-gray-500">{exp.url_pattern || exp.urlPattern}</div>
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap">
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        Посетители: {(exp.stats?.visitors || 0).toLocaleString()}
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        {exp.created_at ? new Date(exp.created_at).toLocaleDateString() : (exp.startDate || '—')}
                                    </td>
